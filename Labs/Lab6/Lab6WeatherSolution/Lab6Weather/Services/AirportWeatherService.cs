...
namespace Lab6Weather.Services;

public class AirportWeatherService : Lab6Weather.AirportWeather.AirportWeatherBase
{
    ...
    public override Task<WeatherReply> GetWeather(AirportRequest request, ServerCallContext context)
    {
        ...
        if (randomNumber < 50)
            throw new RpcException(new Status(StatusCode.Unavailable, "Server problem - airport " + request.AirportCode));
